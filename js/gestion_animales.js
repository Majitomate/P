...
function validarCamposAnimal(id, raza, edad, peso, genero, salud, animales, mensajeError) {
    ...
    const EDAD_MINIMA = 0, EDAD_MAXIMA = 40, PESO_MINIMO = 0, PESO_MAXIMO = 200;
...
function abrirModalEditarAnimal(animal, tarjeta, animales) {
    const modal = crearModal(`
        <h2>Editar Animal</h2>
        <label>Arete del Animal:</label>
        <input type="text" id="editarIdAnimal" value="${animal.id}" disabled>
        <label>Raza:</label>
...
function cargarTabla(tableBody) {
    let registros = JSON.parse(localStorage.getItem("tablaRegistros"));

    // Si no hay datos en localStorage, agregar datos iniciales
